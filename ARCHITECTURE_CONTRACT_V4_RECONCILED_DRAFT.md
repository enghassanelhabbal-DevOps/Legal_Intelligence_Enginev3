# ARCHITECTURE CONTRACT V4 — RECONCILED DRAFT

> **Status:** Draft. This document is not authoritative until explicitly approved by the project owner and promoted to `ARCHITECTURE_CONTRACT.md`.
>
...
4. Several domain contracts described below are target contracts and are not yet fully implemented.
5. GPU resource measurements for the current full hybrid/reranker pipeline on the Quadro M2200 remain outstanding.
...
Before optimization of the production retrieval stack is claimed, a reproducible benchmark must exist for:
...
The duplicated root `app.py` retrieval implementation is a **known cleanup task** and must be removed once the canonical path is proven against the supported deployment modes.
...
Before V4 approval, this register must remain visible.
...
### Stage 3 — Canonical Pipeline Consolidation
...
Before promotion to `ARCHITECTURE_CONTRACT.md`, verify:

1. The document matches actual repository behavior or explicitly labels the gap.
2. Every protected metric has reproducible provenance.
3. CI and production pipeline coverage gaps are visible.
4. No duplicate implementation is treated as an accepted architecture pattern.
5. Domain contracts are staged rather than all required immediately.
6. Legal semantics are explicit and jurisdiction/time aware.
7. Dataset governance prevents leakage and contamination.
8. Arabic evaluation is separated from legal reasoning evaluation.
9. Evidence support is enforceable rather than cosmetic.
10. Resource expectations are measurable.
11. Failure handling is bounded and reversible.
12. Claude review is mandatory but not autonomous production self-modification.
13. No new infrastructure is introduced without workload evidence.
14. The roadmap can support Egypt first and later jurisdiction packs.
15. The document remains maintainable and does not duplicate implementation documentation unnecessarily.
