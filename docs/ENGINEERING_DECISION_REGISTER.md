...
## DR-030 — `torch`/`faiss-cpu`/`sentence-transformers` demoted from core dependencies to a `dense` extra; `requests`/`psutil` promoted to core

Status: accepted.

Decision: `pyproject.toml`'s core `dependencies` list previously included `torch`, `sentence-transformers`, and `faiss-cpu` unconditionally — a bare `pip install -e .`, or even `pip install -e '.[streamlit]'`, pulled in the full torch/CUDA-adjacent chain regardless of whether the deployment ever uses local dense retrieval, reranking, or Qwen generation. Fixed by moving those three into a new `dense` extra; `qwen` now depends on `dense` via a self-referencing extra (`legal-intelligence-engine[dense]`) rather than duplicating the pin. `requests` (needed by the remote-generation backend) and `psutil` (DR-029) are now core.

Verified the code side was already mostly ready (PEP 562 lazy `__getattr__` already existed in `retrieval/__init__.py`), but found and closed two real gaps: (1) `reranking/__init__.py` eagerly imported `cross_encoder.Reranker` at package-import time, forcing torch even though the reranker is disabled by policy under `CPU_MINIMAL` — fixed by applying the same lazy-`__getattr__` pattern. (2) `services/query_service.py` had a module-level `from src.legal_ai.retrieval import prepare_pipeline` — a top-level `from X import Y` triggers `X`'s `__getattr__` immediately regardless of how lazy `X` tries to be, defeating the lazy loading entirely and forcing torch on every `import query_service`. Fixed by moving the import inside `QueryService.__init__` (and the new `QueryService.from_plan`, DR-031).

Consequence: `import src.legal_ai.cli`, `import src.legal_ai.services.query_service`, and `from src.legal_ai.runtime import resolve_runtime_plan` are verified torch-free at import time (an explicit `assert "torch" not in sys.modules` smoke test, actually run in a torch-less sandbox). Accessing `DenseIndex`/`HybridRetriever`/`Reranker`/`prepare_pipeline` still requires the `dense` extra and raises a clear `ImportError` naming `pip install -e '.[dense]'` rather than a raw `ModuleNotFoundError`.

## DR-031 — `ResolvedRuntimePlan`: device/batch/candidate/resource policy centrally owned instead of independently guessed per layer

Status: accepted.

Decision: previously, `RuntimeConfig`/`PipelineConfig` (`core/models.py`) were constructed with static, hand-set defaults independent of what `runtime.hardware`/`runtime.profiles`/`runtime.budgets` actually discovered, and `retrieval/pipeline.py`'s `configure_runtime()` made its own in-process `torch.cuda.is_available()` call — a second, unprotected probe bypassing the isolated-subprocess safety `probe_cuda()` already provided (DR-028). New `src/legal_ai/runtime/plan.py` adds `ResolvedRuntimePlan` resolved exactly once via `resolve_runtime_plan()` (the existing discovery chain, unchanged) plus `to_runtime_config()`/`to_pipeline_config()`/`to_generation_config()` translation methods deriving the config shapes existing code already consumes — pure translations, no retrieval/ranking/generation algorithm changed.

Wired via: `configure_runtime()` gained an optional `known_cuda_available` parameter (default `None`, preserving prior behavior for callers not opting in); `prepare_pipeline()` threads it through; new `prepare_pipeline_from_plan()` wraps it with plan-derived config; new `QueryService.from_plan()` classmethod added alongside the existing hand-configured constructor (left untouched).

Consequence: verified torch-free and exercised for all four `ExecutionProfile` values via `scripts/runtime_benchmark.py` (DR-033) on real CI-sandbox hardware. `ACCELERATED`'s device-resolution branch is covered by a unit test using a constructed `HardwareSnapshot` fixture rather than real GPU hardware, since none is available here — a disclosed gap, not a silently-assumed pass.

## DR-032 — Bounded execution/backpressure (`BoundedExecutor`) and lightweight CI restored on `feat/enterprise-foundation`

Status: accepted.

Decision: `ResourceBudget.max_workers`/`max_queue_size` were config values with no enforcement mechanism consuming them. `concurrent.futures.ThreadPoolExecutor` only bounds concurrently-*running* tasks; its internal work queue has no `maxsize`, so `.submit()` past that count still succeeds and silently accumulates unbounded pending work. New `src/legal_ai/runtime/execution.py` adds `BoundedExecutor`: a `ThreadPoolExecutor` gated by a `threading.BoundedSemaphore` sized to `max_workers + max_queue_size`, so total in-flight work is a hard, finite ceiling. `submit(..., block=False)` (default) raises `BackpressureRejected` immediately at capacity; `block=True` blocks instead. Verified under real concurrent load in `tests/test_execution.py` (event-gated, not sleep-based) and in `scripts/runtime_benchmark.py`, which deliberately oversubmits and asserts `BackpressureRejected` actually fired.

Separately, `.github/workflows/ci.yml`'s triggers listed only `developing`/`master` — `feat/enterprise-foundation` was never in that list, so CI has never run against this branch. This explains a confirmed finding: running `ruff check src api ui scripts` against this branch's `HEAD` surfaced 20 pre-existing violations across `api/app.py`, `ui/streamlit_app.py`, `scripts/quality_gate.py`, `scripts/regression_harness.py`, `src/legal_ai/cli.py`, `generation/manager.py`, both generation backends, and `retrieval/pipeline.py` — none introduced by this round, all pre-existing and uncaught because CI had never run. All 20 fixed (line-wrapping/import-order only). One fix was substantive: `qwen_transformers_backend.py`'s Arabic system prompt had a stray `# noqa: E501` sitting *inside* the triple-quoted string literal (not a real comment — it did nothing for lint and was literally being sent to the LLM as part of the prompt); removed and the line properly wrapped.

`feat/enterprise-foundation` added to CI triggers; `psutil` added to the install step; a new `mypy` step scoped to `src/legal_ai/{runtime,domain,evaluation,core}` added — deliberately, not by oversight: mypy statically follows every import it finds including lazily-deferred ones, so type-checking `retrieval`/`reranking`/`generation` would require torch/faiss/sentence-transformers installed, violating this job's "no heavy ML deps, no GPU" contract (confirmed by trial: pointing mypy at `services/` pulled in `qwen_transformers_backend.py` and surfaced 3 pre-existing unrelated errors). All four governed packages verified clean before wiring the gate in.

## DR-033 — Deterministic adaptive batch policy (`runtime.adaptive_batch`) and Stage 2 runtime benchmark report

Status: accepted.

Decision: `runtime.faults`'s `OUT_OF_MEMORY -> REDUCE_BATCH_AND_RETRY` policy bounds retry *count* but never specified what batch size to retry with. New `src/legal_ai/runtime/adaptive_batch.py` adds `AdaptiveBatchState` + `on_out_of_memory()`/`on_success()` pure transition functions: deterministic (no `random`, no wall-clock input — replay-tested), monotonic (halves toward `floor`, requires `RECOVERY_STREAK_REQUIRED=3` consecutive successes before doubling back up, never past `ceiling`), and bounded by the `ceiling` supplied from `ResourceBudget` at `initial_state()` — only shrinks within what `budget_for_profile()` already decided was safe.

New `scripts/runtime_benchmark.py` produces `artifacts/reports/runtime_benchmark.json`: real, measured output covering (1) `discover_hardware()` wall-clock cost including the isolated-subprocess CUDA probe, (2) every `ExecutionProfile`'s actually-resolved `ResolvedRuntimePlan` and derived configs, (3) a synthetic oversubmission through `BoundedExecutor` asserting `BackpressureRejected` fired, (4) a simulated OOM/success sequence with a determinism replay check. Exits non-zero if either assertion fails, so it is wired into `ci.yml` as a gate, not just a report generator. Run on this project's CI-sandbox host (1 effective CPU core, no GPU): `BALANCED`'s `max_workers` resolved to `2` (topology-scaled per DR-029, not the static baseline of `4`), `ACCELERATED`'s device stayed `cpu` (no GPU present — correctly conservative), both assertions passed. 26 new tests added across `test_adaptive_batch.py`, `test_execution.py`, and `test_runtime_plan.py`; full suite passes clean.

Consequence (disclosed honestly): `adaptive_batch`'s transition functions are exercised only against simulated sequences — no real embedding/reranking call site in this codebase yet calls `on_out_of_memory()`/`on_success()` in response to an actual `torch.cuda.OutOfMemoryError`; wiring that real call site is follow-up work, not claimed done here. Similarly `BoundedExecutor` is a verified, ready-to-use primitive, not yet integrated into `dense.py`/`hybrid.py`/`manager.py`'s actual call sites — wiring it in is the next Stage 2 follow-through, explicitly out of scope for this delivery ("do not rewrite retrieval semantics").

## DR-034 — Final integration gate: `BoundedExecutor`/adaptive batch wired into the real call paths; `qwen_transformers_backend.py` excluded from the mypy governed gate

Status: accepted.

Decision: DR-033 explicitly disclosed that `BoundedExecutor` and `adaptive_batch` were verified primitives not yet wired into any real call site. This closes that gap without touching retrieval/ranking/generation semantics:

- **`BoundedExecutor` -> `api/app.py`**: the `/v1/query` endpoint previously called `service.answer()` synchronously inside an `async def` route — a blocking, CPU/GPU-bound call executed directly on the event-loop thread, serializing every concurrent request and providing no bound on concurrent work. Now `service.answer` is submitted via `executor.submit(..., block=False)` and awaited with `asyncio.wrap_future`; `BackpressureRejected` maps to HTTP 503. The executor is constructed once at startup from `ResolvedRuntimePlan.budget` (`plan.budget.max_workers + max_queue_size`), not a separate hard-coded number. The old hand-built `RuntimeConfig()`/`PipelineConfig()` + a separate `LOAD_RERANKER` env var were removed in favor of `QueryService.from_plan()`, which derives reranker-loading from `plan.budget.rerank_batch_size > 0` — eliminating the last duplicate, parallel policy lever outside `ResolvedRuntimePlan` (item 4).

- **`adaptive_batch` -> `DenseEncoder.encode_documents()` / `Reranker.score()`**: new `src/legal_ai/runtime/torch_adaptive_batch.py` bridges the torch-free `adaptive_batch` state machine to real `torch.cuda.OutOfMemoryError`/CPU `MemoryError`/OOM-text `RuntimeError` exceptions. Both methods now hold a persistent `AdaptiveBatchState` (ceiling = the caller-supplied `batch_size`, unchanged call signature) and route the actual `model.encode()`/`model.predict()` call through `run_batch_with_adaptive_policy()`, which halves deterministically and retries bounded by `runtime.faults`'s registered `OUT_OF_MEMORY` `RecoveryPolicy.max_attempts` (1), then re-raises if still exhausted — never an unbounded loop. Verified against **real torch** (installed in a verification sandbox after a `--no-deps` workaround for a disk-space failure on the default CUDA wheel): a deliberately flaky fake encoder OOMs once at the ceiling, steps 32->16 deterministically, and returns embeddings identical in value to a non-flaky run — batch-size adaptation cannot change retrieval scoring, only how many attempts at what size are made. `torch_adaptive_batch.py` is deliberately NOT re-exported from `runtime/__init__.py`, preserving the torch-free governed-modules gate (DR-032) — only `dense.py`/`cross_encoder.py` (which already hard-require torch) import it directly.

- **Bypass-detection tests** (item 7): `tests/test_api_bounded_execution.py::test_query_endpoint_bypassing_executor_is_detected` replaces `executor.submit` with a poison function and asserts the real endpoint actually calls it; `tests/test_dense_reranker_adaptive_batch.py::test_encode_documents_bypass_detection` / `test_reranker_score_bypass_detection` do the same for `run_batch_with_adaptive_policy`. All three fail if a future refactor reintroduces a direct `service.answer()`/`model.encode()`/`model.predict()` call that skips the bounded/adaptive wrapper.

- **mypy governed-gate regression found and fixed**: Hassan's own `src/legal_ai/__init__.py` addition (`from . import core, evaluation, evidence, generation, ingestion, knowledge, reranking, retrieval, services`, applied directly to the remote branch) caused mypy's static import-following to reach `generation/backends/qwen_transformers_backend.py` even when only checking `runtime`/`domain`/`evaluation`/`core` — because those packages are subpackages of `legal_ai`, and mypy loads the parent `__init__.py` first, which now eagerly imports `generation`. Fixed with a scoped `[[tool.mypy.overrides]] module = "src.legal_ai.generation.backends.*" ignore_errors = true` — precise, documented, and does not weaken the gate for any module that is actually part of it.

Consequence: `scripts/runtime_benchmark.py` gained two new sections proving the *integrated* paths, not just the standalone primitives (DR-033) — `integrated_api_path` (the real `/v1/query` coroutine under real concurrent load, asserting the second request gets HTTP 503) and `integrated_dense_reranker_path` (the real `DenseEncoder`/`Reranker` methods with a simulated OOM, asserting deterministic step-down). The latter requires the `dense` extra and self-reports `{"skipped": true, "reason": ...}` when unavailable — verified both ways (with the extra installed, and via a simulated blocked-`import torch`). `.github/workflows/ci.yml`'s pytest step now also excludes the new `tests/test_dense_reranker_adaptive_batch.py` (6 tests) alongside the existing `tests/test_retrieval.py` (3 tests) exclusion, for the identical reason.

## DR-035 — Root-caused and fixed the "bare `pytest` fails, `python3 -m pytest` passes" discrepancy (open since the first Stage 2 session)

Status: accepted.

Decision: this was flagged as an unresolved blocking issue in the very first Stage 2 delivery session and never root-caused. Root cause, found this round: `python -m pytest` gets `sys.path[0] = cwd` for free from Python's own `-m` module-execution semantics; the standalone `pytest` console-script's `sys.path[0]` is instead the directory containing the *script itself* (e.g. `/usr/local/bin`), not the caller's working directory. Since this project's test files do `from src.legal_ai... import ...` relying on the project root being on `sys.path`, that resolution only ever worked *by accident*, depending entirely on which of the two equivalent-looking invocation styles was used — `.github/workflows/ci.yml` invokes the bare `pytest` command, meaning this discrepancy was latent in CI the whole time, not merely a local dev-environment quirk.

Fixed with pytest's own built-in mechanism for exactly this situation: `pythonpath = ["."]` under `[tool.pytest.ini_options]` in `pyproject.toml` (available since pytest 7.0; this project pins pytest>=8). This makes both invocation styles resolve `import src...` identically, without requiring `pip install -e .` or a `conftest.py` `sys.path.insert` hack.

Verified directly: `pytest tests/ --ignore=tests/test_retrieval.py --ignore=tests/test_dense_reranker_adaptive_batch.py -v` (the literal CI command) failed with `ModuleNotFoundError: No module named 'src'` across 20 files before this fix, and passes (239/239) after it, on the same host, same environment, changing nothing else. Full suite (`pytest tests/ -q`, all 248 tests, dense extra installed) also verified clean under the bare `pytest` invocation. `ruff`, `mypy`, and every `scripts/*.py` CI step were also re-verified under their bare (non-`python -m`) invocations to rule out the same class of issue elsewhere — none found.

## DR-036 — Lightweight-safe shutdown; explicit model-concurrency gate separate from request admission

Status: accepted.

Decision: **Lightweight shutdown (Risk 2).** `QueryService.close()` previously did an unconditional `import torch` and called `torch.cuda.is_available()` directly — meaning a REMOTE_LLM/lightweight deployment with no `dense` extra installed would crash on shutdown with `ModuleNotFoundError`, and the check duplicated exactly the unprotected in-process CUDA probe pattern DR-028 eliminated elsewhere. Fixed: `close()` now only imports torch, and only calls `torch.cuda.empty_cache()`, if a torch-backed component (encoder/reranker) was actually loaded onto that `QueryService` instance — driven by what is actually resident, not a fresh global hardware re-check. Per-component cleanup failures are logged and shutdown continues; they are not swallowed silently everywhere (narrow per-attribute `try/except`), nor allowed to crash the process.

**Model concurrency (Risk 3).** Request-admission concurrency (`BoundedExecutor`, DR-032) does not imply it is safe to run several concurrent inferences against the SAME loaded model object — two threads calling a GPU-resident model's forward pass concurrently risks VRAM OOM, CUDA allocator fragmentation, and unpredictable latency independent of correctness. Added `ResourceBudget.max_model_concurrency` / `RuntimeConfig.max_model_concurrency` (conservative default: 1, for every profile — no profile-specific tuning attempted this round, matching "use conservative defaults, avoid speculative configuration explosion") and a new `runtime.execution.ModelConcurrencyGate` (a `threading.Semaphore` context manager, no scheduler). Wired into `DenseEncoder.encode_documents()`/`encode_query()` and `Reranker.score()`, each holding its own gate instance sized from the resolved plan — NOT applied to BM25 (thread-safe, lightweight in-memory lookup, not a heavy model object) or remote generation (each call is an independent HTTP connection, already bounded by `max_workers`). Local Qwen generation backend (`qwen_transformers_backend.py`) was NOT wired with this gate this round — explicitly out of scope per "do not rewrite LLMManager"; disclosed as a remaining gap, not silently treated as covered.

Verified with 5 new tests (`tests/test_model_concurrency.py`) using instrumented fake models with real thread contention (not sleep-based flakiness) proving simultaneous entry is actually prevented at `max_concurrency=1`, plus a negative control at `max_concurrency=2` proving the test would have caught the gate's absence.

## DR-037 — Docker/packaging: explicit `INSTALL_EXTRAS` build mode instead of an always-bare install

Status: accepted.

Decision: DR-030 correctly moved `torch`/`faiss-cpu`/`sentence-transformers` out of core `dependencies` into a `dense` extra, but the `Dockerfile` still did a bare `pip install -e "."` while the default `CMD` runs the API, whose default `ExecutionProfile` (BALANCED/ACCELERATED, resolved from discovered hardware) uses local dense retrieval by default — producing a container that builds successfully and then crashes at startup with `ModuleNotFoundError` the moment a query is served. Fixed with an explicit `ARG INSTALL_EXTRAS=dense` in the `Dockerfile`, conditionally appended to the install command; `docker-compose.yml`'s `api` service pins `INSTALL_EXTRAS: dense` (serves canonical retrieval) while `streamlit` pins `INSTALL_EXTRAS: streamlit` (that service runs in remote/HTTP-client mode against the `api` service per its own module docstring — it never does local retrieval and needs no dense stack at all).

Consequence: Docker itself is unavailable in this sandbox (`DOCKER_RUNTIME_UNVERIFIED_CURRENT_ENVIRONMENT` — reproduction: `docker build -t legal-rag-engine:test .` then `docker build --build-arg INSTALL_EXTRAS= -t legal-rag-engine:lightweight .` and diff the two images' installed packages). Verified the underlying dependency-resolution claim directly instead, per the fallback this task itself allows: a genuinely clean venv with a bare `pip install -e .` (no extras) has zero `torch`/`faiss`/`sentence-transformers` installed, and every import `src.legal_ai.runtime`/`src.legal_ai.services.query_service`/`src.legal_ai.generation` still succeeds (item 24); separately, resolving a dense-requiring `ExecutionProfile` in that same venv and constructing `QueryService.from_plan()` raises a single, clear, actionable `ImportError` naming `pip install -e '.[dense]'` — never a silent architecture downgrade to a different retrieval method (item 25). `api/app.py`'s `/v1/ready` already surfaces `_state["startup_error"]` verbatim, so this failure is visible at the readiness endpoint, not just in logs.

## DR-038 — Two-tier CI (dense CPU integration job); OOM-classification/output-order test coverage; repo-root transfer-artifact cleanup

Status: accepted.

Decision: **Two-tier CI (Risk 5).** Stage 2 now directly changes `dense.py`/`cross_encoder.py`/`torch_adaptive_batch.py`, none of which the lightweight CI tier can exercise (it deliberately never installs torch/faiss). Added a second `.github/workflows/ci.yml` job, `dense-cpu-integration`: installs ordinary CPU-only PyTorch (`--index-url https://download.pytorch.org/whl/cpu`, explicitly no CUDA toolkit/GPU FAISS/NVIDIA runtime) plus `pip install -e ".[dense,dev]"`, then runs `tests/test_retrieval.py` + `tests/test_dense_reranker_adaptive_batch.py` + `tests/test_model_concurrency.py` (all model calls faked/stubbed — no network, no real model-weight downloads) plus an assertion that `runtime_benchmark.py`'s `integrated_dense_reranker_path` section is NOT `skipped` in this tier (it is legitimately skipped in the lightweight tier). `docker-build` now depends on both jobs. Fixed 6 pre-existing lint violations in `tests/test_retrieval.py`, surfaced for the first time now that it is lint-gated.

**OOM classification strictness (items 18-20).** Reviewed `is_out_of_memory_error()`: already narrow (CPU `MemoryError`, `torch.cuda.OutOfMemoryError`, or an exact `"out of memory"` substring match on `RuntimeError` only). Added explicit tests proving `ValueError`/`TypeError`/`KeyError`/a shape-mismatch `RuntimeError` all propagate on the FIRST attempt with the adaptive-batch state left completely untouched — a config/logic bug must never be silently retried as if it were a resource condition. Added parametrized output-order/output-count preservation tests across single-batch, one-OOM-retry, and non-even-final-batch scenarios, confirming `embedding[i]`/`score[i]` still corresponds to `texts[i]`/`pairs[i]` regardless of how the adaptive policy regrouped the work.

**Security review (item 27).** `runtime_benchmark.json`/`HardwareSnapshot.to_dict()` reviewed and confirmed to contain no hostname, username, cwd, home directory, or environment-variable dump — capability-focused only (OS name, kernel version string, core counts, RAM, CUDA availability). Added a permanent regression test (`tests/test_hardware.py::test_hardware_snapshot_never_leaks_hostname_username_or_paths`) rather than relying on a one-time manual check.

**Repo-root transfer-artifact cleanup (item 28).** `*.bundle` files and `SHA256SUMS.txt` at the repository root were confirmed (via `SHA256SUMS.txt`'s own contents, which checksum files under a `transfer/` path that does not exist in the working tree) to be leftover cross-machine delivery artifacts from git-bundle-based patch transfer, not source or release artifacts. Removed from tracked source; `.gitignore` updated (`*.bundle`, `SHA256SUMS.txt`, `/transfer/`) to prevent recurrence.

**`ResolvedRuntimePlan` bypass audit (item 22).** Grepped every `torch.cuda.is_available`/`device="cuda"`/`ThreadPoolExecutor(`/`max_workers=`/`*_batch_size=` occurrence in `src/`+`api/`. Classified: the isolated-subprocess probe script inside `hardware.py` (the one sanctioned direct call — DR-028), `pipeline.configure_runtime()`'s backward-compatible fallback branch (only reached when `known_cuda_available` is not supplied — never reached via the plan-driven path), `core/config.py::set_seed()`'s seeding-only check, and backend-local cleanup in `cross_encoder.py`/`qwen_transformers_backend.py` (both already hard-require torch) are all acceptable per this task's own classification (backend-local execution detail / sanctioned probe / cleanup). Two disclosed, NOT-fixed-this-round gaps: `cli.py` builds its own `RuntimeConfig` from argparse flags for the offline index-building tool rather than defaulting through `ResolvedRuntimePlan` (a deliberate, explicit CLI override surface, not a hidden divergence — but not plan-driven either); `core/config.py::load_runtime_config()` (env-var-driven config loader) is exported but not actually called by any live code path (`api/app.py` uses `resolve_runtime_plan()`; `cli.py` uses argparse directly) — effectively dead/unused, not a live bypass. Neither affects the API request-serving path this hardening pass is about; both are named explicitly rather than left for a future session to rediscover.
